      ...
      >
        <div className="modal-box bg-(--bg-color)">
          ...
          {state.host === state.user ? (
            <p>
              Press continue to create a new lobby, press exit to leave the
              lobby.
            </p>
          ) : state.next === null ? (
            <p>Waiting for the host to create a new room ...</p>
          ) : state.next ? (
            <p>The host has created a new room. Press continue to join it!</p>
          ) : (
            <p>The host has left. Press exit to leave the game.</p>
          )}
          <LittleLeaderboard />
          ...
          <div className="modal-action">
            <form method="dialog" className="flex justify-between w-full">
              {state.host === state.user ? (
                <button className="btn" onClick={handle_continue_host}>
                  Continue
                </button>
              ) : state.next === true ? (
                <button className="btn" onClick={handle_continue_user}>
                  Continue
                </button>
              ) : null}
